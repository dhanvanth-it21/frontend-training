...
task1.addEventListener('blur', titleCase);
...
function task1fun(e) {
    const ascciNum = asciiValue(e.key);
    if(!((ascciNum >= a && ascciNum <= z) || (ascciNum >= A && ascciNum <= Z) || ascciNum === space)){
        e.preventDefault();
    }    
    const val = e.target.value;
    const len = val.length;
...
function titleCase(e) {
    ...
    const titleCased = inputVal.map((word) => {
        return `${word.charAt(0).toUpperCase()}${word.substring(1).toLowerCase()}`;
    });
    e.target.value = titleCased.join(" ");
}
...
function task2fun(e) {
    ...
    const val = e.target.value;
    e.target.value = `${val.charAt(0).toUpperCase()}${val.substring(1).toLowerCase()}`
}
...
function task4fun(e) {
    const ascciNum = asciiValue(e.key);
    if(!((ascciNum >= a && ascciNum <= z) || (ascciNum >= A && ascciNum <= Z) || (ascciNum >= num0 && ascciNum <= num1))){
        e.preventDefault();
    }
    const val = e.target.value;
...
function task3fun(e) {
    ...
    if (!((ascciNum >= num0 && ascciNum <= num1) || (e.key === dot && !val.includes(dot)))) {
        e.preventDefault();
    }
    else {
        // if()
